...
public async Task<bool> FlexMatchSimulatorHandler(Stream stream, ILambdaContext context)
{
    ...
    foreach (var playerProfileConfig in simulation.PlayersConfig)
    {
        ...
        var profile = playerProfiles.Find(x => x.ProfileId == playerProfileConfig.ProfileId);
        var latencyProfile = latencyProfiles.Find(x => x.ProfileId == playerProfileConfig.LatencyProfileId);
        ...
        if (profile != null)
        {
            for (var i = 0; i < playerProfileConfig.NumPlayers; i++)
            {
                ...
                var playerAttributeString = new StringBuilder();
                ...
                foreach (var attribute in profile.Attributes)
                {
                    ...
                    if (attribute.AttributeType == "S")
                    {
                        ...
                        if (att != null)
                        {
                            ...
                            if (playerAttributeString.Length > 0)
                            {
                                playerAttributeString.Append(","); // Add a comma separator
                            }
                            playerAttributeString.Append(att.Value);
                        }
                    }
                    ...
                    else if (attribute.AttributeType == "N" && attribute.ValueType == "randomInteger")
                    {
                        ...
                        if (att != null)
                        {
                            double randomValue = Utils.RandomInt(att.Min, att.Max);
                            playerAttributes[att.AttributeName].N = randomValue;
                            if (playerAttributeString.Length > 0)
                            {
                                playerAttributeString.Append(","); // Add a comma separator
                            }
                            playerAttributeString.Append(randomValue);
                        }
                    }
                    else if (attribute.AttributeType == "N" && attribute.ValueType == "randomDouble")
                    {
                        ...
                        if (att != null)
                        {
                            double randomValue = Utils.RandomDouble(att.Min, att.Max);
                            playerAttributes[att.AttributeName].N = randomValue;
                            if (playerAttributeString.Length > 0)
                            {
                                playerAttributeString.Append(","); // Add a comma separator
                    ...
                    else if (attribute.AttributeType == "SL" && attribute.ValueType == "value")
                    {
                        ...
                        if (att != null)
                        {
                            ...
                            playerAttributeString.Append(string.Join(",", att.Value));
                        }
                    }
                    else if (attribute.AttributeType == "SL" && attribute.ValueType == "randomStringList")
                    {
                        ...
                        if (att != null)
                        {
                            ...
                            att.Value.Shuffle();
                            var randomStringList = att.Value.ToList().GetRange(0, numStrings);
                            playerAttributes[att.AttributeName].SL = randomStringList;
                            if (playerAttributeString.Length > 0)
                            {
                                playerAttributeString.Append(","); // Add a comma separator
                            }
                            playerAttributeString.Append(string.Join(",", randomStringList));
                        }
                    }
                    else if (attribute.AttributeType == "SDM")
                    {
                        ...
                        if (att != null)
                        {
                            ...
                            playerAttributeString.Append(string.Join(",", att.ValueMap.Select(kvp => kvp.Key + ":" + kvp.Value)));
                        }
                    }
                }

                // Convert the StringBuilder to a string
                string csvInput = playerAttributeString.ToString();
                LambdaLogger.Log($"CSV Input: {csvInput}");
                // Create the SageMaker client
                var sageMakerClient = new AmazonSageMakerRuntimeClient();;

                // Create the InvokeEndpointRequest with the CSV input data
                var request = new InvokeEndpointRequest
                {
                    EndpointName = "PlayerSkills-Endpoint", // Replace with your actual endpoint name
                    ContentType = "text/csv", // Set the content type to text/csv
                    Accept = "text/csv", // Set the accept header to text/csv
                ...
                var skillValue = reader.ReadToEnd();

                // Add the skill value to the player attributes
                playerAttributes.Add("skill", new Amazon.GameLift.Model.AttributeValue { N = double.Parse(skillValue) * 100 });
